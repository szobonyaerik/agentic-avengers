...
skipped it reports stage `unknown` rather than `done` or `e2e-author`, which would be verdicts over
phases it never opened. Artifacts are the state, so a run resumes across `/clear`, compaction, or a new
session.
...
- **Preflight also refuses a known-stale plugin, in both modes.** Phases execute the cached release
  ...
  `AVENGER_SOURCE_REPO` configured on this machine) is reported and never enforced. Rule and remedy:
