...
- **Always write the regression test first, and watch it fail.** No regression test = the bug comes back, so this step is not optional — and the order is what makes it worth anything. Per `skills/tdd`: write a test that reproduces the bug at the seam it is observable through, run it and confirm it fails *for the reason you diagnosed*, and only then fix the code. A regression test written after the fix proves nothing except that the code does what it currently does. Record it in `test-mapping.md` against the spec id whose behavior the bug violated.
...
- **You never edit, delete, relax, skip, or `xfail` a locked test** — one from a phase the Verifier has already passed (`pipeline-conventions` §4). Adding your failing regression test is allowed; reshaping an existing test so your fix looks correct is not. If a locked test is wrong, that is a route-back to the Spec Writer for a spec that says so.
